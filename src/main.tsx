...
export const Main = component$(() => {
  ...
  return $(() => {
    return (
      <div
        id="my-app"
        onDocumentExpensiveComputationDone$={() => (state.running = false)}
      >
        <Home />
        <Home />
        <Home />
        ...
        <ol>
          <li>
            ...
            <code>partytown.js</code> is used to execute things of main thread
            and not for rendering the application.)
          </li>
          <li>
            Partytown executes <code>&lt;script&gt;</code> tag that contains a
            simulation of expensive script. The expensive script simplation
            keeps the web-worker thread busy for 2.5 seconds, leaving
            main-thread free for user interactions. (Without Partytown the
            script would block the main thread resulting in poor user
            experience.)
          </li>
          <li>
            Once the expensive operation is finished it dispatches custom event
            (<code>expensiveComputationDone</code>) that this component listens
            on. It is only at that time that Qwik lazy-loads the component
            render function and updates the UI. (See network tab.)
          </li>
          <li>
          ...
          <li>
            Notice that Qwik automatically lazily-loaded and resumed the
